...
##### laço para plotar todos os gráficos 2012
graficosREC12 <- map2(
    # está bugando no Num4 (cosmopolis)
    RMC,
    names(RMC),
    function(cidade, rotuloCIDADE){
      # vendo qual é a cidade no prompt
      print(cidade)
      # grafico 01
      plotar_df <- totREC_df %>% 
        filter(
          ano_eleicao == 2012,
          municipio == rotuloCIDADE
          #municipio == RMC
        ) %>% 
        mutate(
      ...
      #adicionado2 (326)
      intervREC <- seq(0, maxREC, length.out = 8) 
      ...
      plotar_df %>%   
        ...
        geom_bar(
          # adicionado5 (388)
          #aes(y = tamColig),
          aes(y = tamColig, fill=factor('Coligação')),
          stat = "identity",
          alpha = .2,
          width = .25,
      ...
      scale_y_continuous(
        ...
        labs(
          title = str_c("Receita dos Competidores - ", str_to_title(cidade), ", eleição de 2012"),
          x = "COLIGAÇÃO",
        ...
        theme(
          ...
          # adicionado1 (linha 444)
          panel.grid.major.y = element_line(colour = 'yellow', size = 1.75),
...
### !!!
# Cosmopolis (4) e Valinhos (19)
# não aparecem receita dos eleitos 2012
### !!!


##### laço para plotar todos os gráficos 2016
graficosREC16 <- map2(
  # está bugando no Num4 (cosmopolis)
  RMC,
  names(RMC),
  function(cidade, rotuloCIDADE){
    ...
    # nomes barras
    categ_cands <- plotar_df %>% 
      select(cpf_do_candidato, recMUNIC, sitPOSIC16) %>% 
      distinct()
    ...
    categ_cands <- categ_cands %>% 
      mutate(
        sitPOSIC16 = teste_rotulos[sitPOSIC16]
      )
    ...
    plotar_df %>%   
      ...
      geom_text(
        data = categ_cands,
        #aes(label = sitPOSIC16, y = if_else(cpf_do_candidato == "total_receita",recMUNIC, .8*recMUNIC)),
        aes(
          label = case_when(
            str_detect(sitPOSIC16, "ELEITO") ~ str_replace(sitPOSIC16, "]", "º]"),
            str_detect(sitPOSIC16, "Cidade") ~ sitPOSIC16,
            str_detect(sitPOSIC16, c("ELEITO","Cidade"), T) ~ str_c(sitPOSIC16, "º")
          ),
          y = recMUNIC,
          fontface = if_else(str_detect(sitPOSIC16, "ELEITO"), "bold", "plain")
        ),
        #vjust = -2.5,
        vjust = -3.5,
        #hjust = .4,
    ...
    scale_y_continuous(
      ...
      scale_fill_brewer(palette = "Dark2")+
      labs(
        title = str_c("Receita dos Competidores - ", str_to_title(cidade), ", eleição de 2016"),
        x = "COLIGAÇÃO",
        fill = "Origem da Receita",
        y = str_c("Receita  (R$ ", maxREC, ")") %>% str_to_upper()
      )+
      ...
      theme(
        ...
        axis.text.x = element_text(
          face = "bold",
          size = if_else(cidade=="campinas", 10.5, 12)
        )
      )+
      # adicionado6 (450)
      guides(fill=guide_legend(nrow = 2))
...
dir.create("~/imagens/mestrado_EDY_graficos/graficosReceitas_cidades12e16")
...
map2(
  ...
  function(g, nome){
    ggsave(
      filename = str_c("~/imagens/mestrado_EDY_graficos/graficosReceitas_cidades12e16/", nome, "12.jpeg"),
      plot = g,
      width = 10*1.25,
      height = 7*1.25
    )
...
map2(
  ...
  function(g, nome){
    ggsave(
      filename = str_c("~/imagens/mestrado_EDY_graficos/graficosReceitas_cidades12e16/", nome, "16.jpeg"),
      plot = g,
      width = 10*1.25,
      height = 7*1.25
    )
...
# zipando pasta
zip(
  zipfile = "~/imagens/mestrado_EDY_graficos/graficosRecRMC12e16",
  files = dir("~/imagens/mestrado_EDY_graficos/graficosReceitas_cidades12e16/", full.names = T)
)
